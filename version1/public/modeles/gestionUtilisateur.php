...
function AfficheFormGestionUtilisateur(){
	...
	echo "<table border='0' align='center'>"
		."<h1 class='utilisateurs'>Gestion des Utilisateurs:</h1>";
		if(!isset($_POST['triID'])){
			if(!isset($_SESSION['triID'])){
			$tri = array(1,0,0,0);
			}else{
			$tri = array($_SESSION['triID'],$_SESSION['triPseudo'],$_SESSION['triType'],$_SESSION['triStatut']);
			}
		}else{
			...
			$tri = array($_POST['triID'],$_POST['triPseudo'],$_POST['triType'],$_POST['triStatut']);
		}
...
$type = array("super-utilisateur","journaliste","artiste","association","artisans","groupe musical");
$status = array("Nouveau","en Cours","Admis","kick&eacute;","banni","d&eacute;sinscrit");
$couleur = array("FFFFFF","FF5B2B","046380","556627","B9121B","34393E");
$ordonancement = array("dateInscription ASC","id_utilisateur ASC","id_utilisateur DESC","","pseudo ASC","pseudo DESC","","type_compte ASC","type_compte DESC","","statut ASC","statut DESC");
$indiceTri = array("&harr;","&darr;","&uarr;");
if($tri[2]!=0){
	if($tri[1]!=0){
		if($tri[3]!=0){
...
echo "<table border='0'>";
	...
	echo "<tr>";
		...
		echo "<td align='center'><form action='index.php?page=gestionUtilisateur' method='POST'>"
			."<input type='hidden' name='triID' value='".$futurTriID."'>"
			."<input type='hidden' name='triPseudo' value='".$tri[1]."'>"
			."<input type='hidden' name='triType' value='".$tri[2]."'>"
			."<input type='hidden' name='triStatut' value='".$tri[3]."'>"
			."<input type='submit' value='".$indiceTri[$tri[0]]."'></form></td>";
		echo "<td align='center'><form action='index.php?page=gestionUtilisateur' method='POST'>"
			."<input type='hidden' name='triID' value='".$tri[0]."'>"
			."<input type='hidden' name='triPseudo' value='".$futurTriPseudo."'>"
			."<input type='hidden' name='triType' value='".$tri[2]."'>"
			."<input type='hidden' name='triStatut' value='".$tri[3]."'>"
			."<input type='submit' value='".$indiceTri[$tri[1]]."'></form></td>";
		echo "<td align='center'><form action='index.php?page=gestionUtilisateur' method='POST'>";
			if($tri==array(0,0,0,0)){
		...
		echo "<td align='center'><form action='index.php?page=gestionUtilisateur' method='POST'>"
			."<input type='hidden' name='triID' value='".$tri[0]."'>"
			."<input type='hidden' name='triPseudo' value='".$tri[1]."'>"
			."<input type='hidden' name='triType' value='".$futurTriType."'>"
			."<input type='hidden' name='triStatut' value='".$tri[3]."'>"
			."<input type='submit' value='".$indiceTri[$tri[2]]."'></form></td>";
		echo "<td align='center'><form action='index.php?page=gestionUtilisateur' method='POST'>"
			."<input type='hidden' name='triID' value='".$tri[0]."'>"
			."<input type='hidden' name='triPseudo' value='".$tri[1]."'>"
			."<input type='hidden' name='triType' value='".$tri[2]."'>"
			."<input type='hidden' name='triStatut' value='".$futurTriStatut."'>"
			."<input type='submit' value='".$indiceTri[$tri[3]]."'></form></td>";
	...
	while($data = mysql_fetch_assoc($req)){
		...
		if($data['type_compte'] != 0){
			echo 	"<td class='".$class."'>".$data['id_utilisateur']."</td>"
				."<td class='".$class."' style='text-align:left;'><a class='info' onclick='return false' href='#'>".$data['pseudo']
				."<span>";
				...
				$repPerso = SVNRADIEURAE_DIR."upload_utilisateurs/".$data['repertoirePersonnel'];
				
				echo 	"</span></a></td>"
						."<td class='".$class."'>".$data['dateInscription']."</td>"
						."<td class='".$class."'>".$type[$data['type_compte']]."</td>"
						."<td class='".$class."' style='background-color:#".$couleur[$data['statut']].";'>".$status[$data['statut']]."</td>";
				$droits = afficheDroitDossierUtilisateur($repPerso);
				if($droits == 'drwxr-xr-x'){$dircolor = 5;}else if($droits == 'u---------'){$dircolor = 3;}else{$dircolor = 1;}
				echo "<td class='".$class."' style='background-color:#".$couleur[$dircolor].";'>".$data['repertoirePersonnel']."<br />".$droits."</td>";
			if($admin == 0){
				echo "<td><form method='POST' action='controlleurs/traitementSuppressionUtilisateur.php'>"
					."<input type='hidden' name='id' value='".$data['id_utilisateur']."'>"
					."<input type='hidden' name='pseudo' value='".$data['pseudo']."'>"
					."<button type='submit' style='width:33px;heigth:100%;' class='btn_modif'><img src='./images/picto-supprimer_up.gif' onMouseOut='this.src=btn_suppr_up;' onMouseDown='this.src=btn_suppr_down;' onMouseOver='this.src=btn_suppr_hover;' alt='supprimer?' /></button>"
				."</form></td>";
				echo "<td><form method='POST' action='controlleurs/traitementModifEtatUtilisateur.php'>"
					."<input type='hidden' name='id' value='".$data['id_utilisateur']."'>"
					."<button type='submit' style='width:60px;heigth:100%;' class='btn_modif'><img src='./images/picto-modifierCircle_up.gif' onMouseOut='this.src=btn_modif_up;' onMouseDown='this.src=btn_modif_down;' onMouseOver='this.src=btn_modif_hover;' alt='Changer?' /></button>"
				."</form></td>";
			}
		}
		echo "</tr>";		
