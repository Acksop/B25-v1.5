<?php
function AfficheFormValidationDossier(){
	//--
	check_session();
	check_admin_session();
	//--
	...
	if ($nbValidation){
		...
		echo wcode_css();
		if(isset($_GET['id'])){
			$data = mysql_fetch_assoc(recuperationDossierAValider($_GET['id']));
		}else{
			$data = mysql_fetch_assoc(recuperationPremierDossierAValider());
		}
		$wcode = $data['bbcode'];
		...
		if(isset($_GET['modification'])){
			echo "<form method='post' name='dossier' action='controlleurs/traitementValidationDossier.php'>"
			...
			if(isset($data)){
				...
				echo "<p class='corpsDossier'>";
					if ($reponse){
						echo $wc->donner_html();
						echo "<center>";
						echo "<form method='post' style='display:inline;' action='index.php?page=validationDossier&id=".$data['id_dossier']."&modification=oui'>"
						."<input type='hidden' name='reecriture' value='oui'/>"
						."<input type='submit' class='btn_dossiers' value='&Eacute;diter ?' />"
						."</form>";
						echo "<form method='post' style='display:inline;' action='controlleurs/traitementValidationDossier.php'>"
							."<input type='hidden' name='validation_id' value='".$data['id_dossier']."'/>"
							."<input type='submit' class='btn_dossiers' value='Valider et Archiver?' />"
							."</form>";
						echo "<form method='post' style='display:inline;' action='controlleurs/traitementSuppressionValidationDossier.php'>"
							."<input type='hidden' name='validation_id' value='".$data['id_dossier']."'/>"
