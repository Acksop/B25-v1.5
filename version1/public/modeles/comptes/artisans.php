...
echo "VISIBILIT&Eacute;E DE LA PAGE PUBLIQUE:</td>";
	...
	echo "</td><td class='utilisateurs'>"
		."<form name='modifInfoArtisans' method='post' action='controlleurs/traitementModifInfoArtiste.php'>"
		."<input type='hidden' name='voir_tweets' value='".$artiste['voir_tweets']."'/>"
...
echo "<form enctype='multipart/form-data' action='controlleurs/traitementArtisansAjoutLOGO.php' method='POST'>"
...
echo "</span>"
	...
	."</form>"
	."</div>"
	."<div id='BanDL' style='background: gray; display:none;'><img src='images/upload.gif' height='120px' width='500px' alt='banniere de telechargements'/></div>";
if($descriptif['logo'] != ''){
	echo "<div id='assocLOGO' class='conteneurGrandInterieur'><img src='".SVNRADIEURAE_PATH.$descriptif['logo']."' alt='{$artiste['pseudo']}'/></div>";
}
echo "</div>";
//-- LE LOGO - Fin
...
do{
	...
	echo "<div class='conteneurArticle'>"
		."<a name='ancre_article_{$article['id_article']}'></a>"
		."<h2 class='legende'>";
		if($i == 0){
			echo "Mes Articles";
		}else{
	...
	echo "<form enctype='multipart/form-data' action='controlleurs/traitementArtisansAjoutArticle.php' method='POST'>"
		."<p class='post' style='float:right;'>(max 10Mo)</p><p class='titre'>Photo de l'article :<br />"
		."<input type='hidden' name='MAX_FILE_SIZE' value='10485760'/>"
		."<input type='file' size='13' name='image' class='tweet' />"
		."<p class='titre' style='width: 250px;'>Etiquette de l'Article: "
		."<input type='text' size='25' name='libelle' class='tweet' value='{$article['libelle']}'/>"
		."</p>"
		."<p class='titre'>Description:<center>"
		."<textarea rows='6' cols='25' name='description' class='tweet' />"
		.$article['description']
		."</textarea></center></p>"
		."<p class='titre' style='float:right;'>Prix:"
		."<span style='float:right;'><input type='text' size='20' name='prix' class='tweet' style='width:50px;' value='{$article['prix']}'/>"
		."&#128;</span></p><br /><br /><br />"
		."<span style='clear:right; float:right;'>";
	if($article['image'] != ''){
		echo "<input type='submit' value='Modifier' class='tweet' onClick='AfficheBanniereDL_ARTICLE({$article['id_article']})'/>"
		."&nbsp;"
		."<input type='button' value='[Annuler]' class='tweetReset' onClick='AfficheARTICLE({$article['id_article']})' />";
	}else{
	...
	if($article['libelle'] != '' || $article['description'] != '' || $article['prix'] != '' || $article['image'] != ''){
		echo "<form action='controlleurs/traitementSuppressionArticleArtisans.php' method='POST'>"
			."<input type hidden name='id_article' value='{$article['id_article']}' />"
			."&nbsp;"
	...
	echo "</div>"
		."<div id='BanDL_ARTICLE_{$article['id_article']}' style='background: gray; display:none;'><img src='images/upload.gif' height='120px' width='500px' alt='banniere de telechargements'/></div>";
	if($article['image'] != ''){
		echo "<div id='article_{$article['id_article']}' class='conteneurArticleInterieur'><center><img src='".SVNRADIEURAE_PATH.$article['image']."' alt='{$article['libelle']}'/><p class='titre' align='center'>{$article['libelle']}</p><p class='article'>{$article['description']}</p><p class='titre' style='float:right;'>{$article['prix']}&nbsp;&#128;</p><br /><br /><br /></center></div>";
	}
echo "</div>";
if ($j == 2){
	echo "</div>";
