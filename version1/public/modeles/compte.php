...
$header_title = "Besan&ccedil;on 25 - Votre compte priv&eacute; sur la Plate-forme";
$header_description = "Votre espace personnel sur Besan&ccedil;on 25";
$header_identifier_url = "besancon25.fr/compte_personnel";
$header_keywords = "Besan&ccedil;on, Besancon, 25000, 25, compte personnel, espace priv&eacute;";


function LancerAffichageDuCorps(){
	echo "<h1 class='utilisateurs'>Mes Pr&eacute;f&eacute;rences ";
	
	$status_compte = array("Nouveau","en Cours","Admis","kick&eacute;","banni","d&eacute;sinscrit");
	$couleur_status = array("FFFFFF","999999","CCCCCC","00FF00","FF0000","0000FF");
...
/*************************************************************************************************************************************************************SUPER UTILE*/
		echo "super-utilisateur .</h1>";
			include 'comptes/super-utilisateur.php' ;
		break;
		case 1:
/************************************************************************************************************************************************************JOURNALISTES*/
		echo "journaliste .</h1>";
		echo "<div class='conteneurGrand'>"
			."<h2 class='legende'>Utilisation :</h2>"
			."<ul class='preferences'>"
			."<li>Peut proposer des <a href='index.php?page=ecritureArticle#corpsPage'>articles</a> soumis a validation</li>"
				."<ul>"
				."<li>Les articles ne sont compos&eacute;s que d'une image dessin&eacute;e au crayon puis scann&eacute;e</li>"
				."<li>Les articles ne sont formes par rien d'autre qu'un texte succint pouvant avoir des liens hypertextes.</li>"
				."<li>Peut envoyer les articles qu'il a &eacute;crit, &agrave; la validation du r&eacute;dacteur en chef pour qu'il soit mis en ligne</li>"
					."<ul>"
					."<li>Vous avez <a href='index.php?page=autorisationArticle#corpsPage'>".recuperationNbAutorisationsArticles($_SESSION['id_utilisateur'])." ARTICLES</a> en attente....</li>"
					."<li>Vous avez &eacute;crit <a href='index.php?page=archiveArticlesJournaliste#corpsPage'>".recuperationNbArticlesValide($_SESSION['id_utilisateur'])." ARTICLES</a> visible sur le site.</li>"
					."<li>Peut administrer les commentaires associ&eacute;s &agrave; ses &eacute;crits (Articles seulement...)</li>"
					."</ul>"
				."</ul>"
			."<li>Peut proposer des <a href='index.php?page=ecritureDossier#corpsPage'>dossiers</a> soumis a validation</li>"
				."<ul>"
				."<li>Les dossiers sont composes de texte formates HTML,d'images, de son et de video[link&eacute;es]</li>"
					."<ul>"
					."<li>Vous avez <a href='index.php?page=archiveDossiersJournaliste'>".recuperationNbDossiersArchiver($_SESSION['id_utilisateur'])." DOSSIER(S)</a> archiv&eacute;(s) sur le site...</li>"
					."<li>Vous avez <a href='index.php?page=archiveDossiersEnValidationsJournaliste'>".recuperationNbDossiersEnValidation($_SESSION['id_utilisateur'])." DOSSIER(S)</a> en attente de validation(s) sur le site...</li>"
					."<li>Vous &ecirc:tes en train d'&eacute;crire <a href='index.php?page=archiveDossiersSauvegardeJournaliste'>".recuperationNbDossiersSauvegarder($_SESSION['id_utilisateur'])." DOSSIER(S)</a> en attente de finalisation(s)...</li>"
					."</ul>"
				."</ul>"
			."<li>Peut <a href='index.php?page=ajoutUtilisateur'>cr&eacute;&eacute;r</a> un compte pour des artistes ou des associations int&egrave;r&eacute;ss&eacute;s ou pour des amis qui veulent participer en tant que journalistes.</li>"
			."<li>Peut <a href='index.php?page=relierDesComptes'>LIER</a> son compte journaliste &agrave; d'autres compte pour une meilleure gestion</li>"
			."<li>Peut annuler son compte, mais laisse les articles/dossiers &eacute;crits et valide dans les archives.</li>"
			."</ul>";
			if( estCompteRelier($_SESSION['id_utilisateur']) ){
				echo "<ul class='preferences'>"
					."<li><b>Ce compte est reli&eacute; au(x) compte(s):</b></li>";
				AfficherLesComptesRelies($_SESSION['id_utilisateur']);
				echo "</ul>";
			}
			echo "</div>";
			$req_journaliste = recuperationInfoJournaliste($_SESSION['id_utilisateur']);
			$journaliste = mysql_fetch_assoc($req_journaliste);
			if(isset($_GET['modif'])){
			$modif = $_GET['modif'];
			$valeur = $_GET['valeur'];
			}else{
			...
			if($journaliste['signature']=="" || $modif == "signature"){
				...
				echo "TELEPHONE DU JOURNALISTE: </td><td class='utilisateurs'>".$journaliste['signature']."</td><td class='utilisateurs'><a href='index.php?page=compte&modif=signature&valeur=".$journaliste['signature']."#ancre_formulaire'>Modifier</a>";
			}			
			echo "</td></tr><tr><td class='titreTableau'>";
...
/*****************************************************************************************************************************************************************ARTISTES*/
		...
		$artiste = mysql_fetch_assoc($req_artiste);
		...
		if($_SESSION['type_compte'] == 2 ){
			echo "artiste ."
				...
				."Voir la page Personnelle sous la forme : "
				."<a href='index.php?page=tableauInscrit&id={$id_artiste}&type={$_SESSION['type_compte']}'>Tablo&icirc;d ?</a>"
				." ou <a href='index.php?page=murInscrit&id={$id_artiste}&type={$_SESSION['type_compte']}'>Mesure&Ocirc;M&egrave;tre ?</a>"
				."</span></h1>";

			echo "<center>";
			include 'comptes/artiste.php';
		}else{
			echo "artisans."
				."<span style='float:right;font-size:10px;'>"
				."<a href='index.php?page=presentationArtisans&id={$id_artiste}'>Voir la page Personnelle?</a>"
				."</span></h1>";
			echo "<center>";
			
			include 'comptes/artisans.php';
		}
			...
			."<h2 class='legende'>Utilisation :</h2>"
			."<ul class='preferences'>"
			."<li>Peut ecrire des <a href='".PAGEB25COM."index.php?page=ecritureAnnonce&id=".$id_artiste."'>annonces pour vendre</a> ses objets d'arts.</li>"
			."<li>Peut <a href='".PAGEB25COM."index.php?page=voirAnnonces_utilisateur&id=".$id_artiste."&cleMap=".calculCleMapAnnoncesUtilisateur($id_artiste)."'>visualiser et/ou supprimer</a> ses ".recuperationNbAnnoncesActivesUtilisateur($id_artiste)." Petites-Annonces actives .</li>"
			."<li>Peut <a href='".PAGEB25COM."index.php?page=voirAnnoncesEnAttentes_utilisateur&id=".$id_artiste."&cleMap=".calculCleMapAnnoncesEnAttentesUtilisateur($id_artiste)."'>&eacute;diter</a> ses ".recuperationNbAnnoncesEnAttenteUtilisateur($id_artiste)." Petites-Annonces en attente de validation.</li>"
			."<li>Peut ecrire sur son compte autant de billet qu'il le souhaite</li>"
			."<li>Peut modifier ses preferences</li>"
				."<ul>"
				."<li>Peut creer des cat&eacute;gories pour ses billets</li>"
				."<li>Peut creer/modifier ses billets &agrave; volont&eacute;</li>"
				."<li>Peut effectuer une modification de son css, par un formulaire precis enregistr&eacute; dans un dossier &agrave; son nom</li>"
				."<li>Peut ajouter 100~ images dans le m&ecirc;me dossier, pour ecrire les billets</li>"
				."<li>Peut ajouter des fichiers musicaux dans le m&ecirc;me dossier, pour ecrire des billets</li>"
				."</ul>";
			echo "<li>Peut annuler son compte, mais laisse les images,les fichiers musicaux et le css sur le serveur.</li>"
			."</ul>";
			if( estCompteRelier($_SESSION['id_utilisateur']) ){
				echo "<ul class='preferences'>"
					."<li><b>Ce compte est reli&eacute; au(x) compte(s):</b></li>";
			...
			echo "<div class='conteneurGrand'>"
			."<h2 class='legende'>Renseignements G&eacute;n&eacute;raux :</h2>"
			."<h6 style='text-align:right;color:white;'>Ces renseignements sont ceux qui apparaissent dans la <a href='./index.php?page=artistes'>liste</a> accessible sur l'interface publique du site.<br />Ils sont n&eacute;cessaire pour que le compte soit valid&eacute;...<br />Votre compte a le statut :"
			."<span style='color:#".$couleur_status[$_SESSION['status_compte']].";'>".$status_compte[$_SESSION['status_compte']]."</span>"
			."</h6>";
			echo "<a name='ancre_formulaire'>&nbsp;</a>";
			if(isset($_GET['lecture'])){
				...
				if($artiste['description']=="" || $modif == "description"){
					...
					echo "DESCRIPTION DE L'ART ou DE L'ARTISANAT: </td><td class='utilisateurs'>".$artiste['description']."</td><td class='utilisateurs'><a href='index.php?page=compte&modif=description&valeur=".$artiste['description']."#ancre_formulaire'>Modifier</a>";
				}
				echo "</td></tr><tr><td class='titreTableau'>";
				...
				echo "VISIBILIT&Eacute;E DU NUM&Eacute;RO DE T&Eacute;LEPHONE :</td>";
					...
					echo "</td><td class='utilisateurs'>"
						."<form name='modifInfoArtiste' method='post' action='controlleurs/traitementModifInfoArtiste.php'>"
						."<input type='hidden' name='voir_telephone' value='".$artiste['voir_telephone']."#formulaire'/>"
				...
				echo "VISIBILIT&Eacute;E DE TOUTES LES COORDONN&Eacute;ES :</td>";
					...
					echo "</td><td class='utilisateurs'>"
						."<form name='modifInfoArtiste' method='post' action='controlleurs/traitementModifInfoArtiste.php'>"
						."<input type='hidden' name='site_web_only' value='".$artiste['site_web_only']."'/>"
						."<button type='submit' class='btn_modif'><img src='./images/picto-modifierCircle_up.gif' onMouseOut='this.src=btn_modif_up;' onMouseDown='this.src=btn_modif_down;' onMouseOver='this.src=btn_modif_hover;' alt='Changer?' /></button>"
						."</form>";
				echo "</td></tr><tr><td class='titreTableau'>";
				echo "VISIBILIT&Eacute;E DU COURRIEL :</td>";
					...
					echo "</td><td class='utilisateurs'>"
						."<form name='modifInfoArtiste' method='post' action='controlleurs/traitementModifInfoArtiste.php'>"
						."<input type='hidden' name='voir_courriel' value='".$artiste['voir_courriel']."'/>"
						."<button type='submit' class='btn_modif'><img src='./images/picto-modifierCircle_up.gif' onMouseOut='this.src=btn_modif_up;' onMouseDown='this.src=btn_modif_down;' onMouseOver='this.src=btn_modif_hover;' alt='Changer?' /></button>"
						."</form>";
				echo "</td></tr><tr><td class='titreTableau'>";
...
/*************************************************************************************************************************************************************ASSOCIATIONS*/
		...
		$association = mysql_fetch_assoc($req_association);
		
		if($_SESSION['type_compte'] == 3 ){	
		
			$req_descriptif = recuperationDescriptifAsso($id_association);
			$descriptif = mysql_fetch_assoc($req_descriptif);
			$req_status = recuperationStatusAsso($id_association);
			$status = mysql_fetch_assoc($req_status);
			$req_membres = recuperationMembresAsso($id_association);
			$nbMembres = mysql_num_rows($req_membres);
			$req_liens = recuperationLiensWebAsso($id_association);
			$nbLiens = mysql_num_rows($req_liens);
			
			echo "association ."
			."<span style='float:right;font-size:10px;'>"
			."<a href='index.php?page=presentationAssociation&id={$id_association}'>Voir la page de pr&eacute;sentation</a>"
			."</span></h1>";
			
			include 'comptes/association.php';
		}else{
			echo "groupe.</h1>";
		...
		echo "<div class='conteneurGrand'>"
			."<h2 class='legende'>Utilisation :</h2>"
			."<ul class='preferences'>"
			."<li>Peut &eacute;crire sur son compte autant de billet qu'il le souhaite</li>"
			."<li>Peut ecrire des <a href='".PAGEB25NET."/index.php?page=ecritureEvenement&id=".$id_association."'>&eacute;v&egrave;nements</a> pour pr&eacute;senter ses activit&eacute;s.</li>"
			."<li>Peut <a href='".PAGEB25NET."index.php?page=voirEvenements_utilisateur&id=".$id_association."&cleMap=".calculCleMapEvenementsUtilisateur($id_association)."'>visualiser et/ou supprimer</a> ses ".recuperationNbEvenementsActivesUtilisateur($id_association)." &Eacute;v&egrave;nements actifs .</li>"
			."<li>Peut <a href='".PAGEB25NET."index.php?page=voirEvenementsEnAttentes_utilisateur&id=".$id_association."&cleMap=".calculCleMapEvenementsEnAttentesUtilisateur($id_association)."'>&eacute;diter</a> ses ".recuperationNbEvenementsEnAttenteUtilisateur($id_association)." &Eacute;v&egrave;nements en attente de validation.</li>"
			
			."<li>Peut modifier ses pref&ecirc;rences</li>"
				."<ul>"
				."<li><span style='text-decoration: line-through;'>Peut afficher son logo, sa mission, ses membres et ses coordonn&eacute;es en haut du blog</span></li>"
				."<li>Peut effectuer une modification de son css, par un formulaire pr&eacute;cis enregistr&eacute; dans un dossier &agrave; son nom</li>"
				."<li>Peut ajouter 100~ images dans le m&ecirc;me dossier, pour &eacute;crire les billets</li>"
				."<li>Peut ajouter des fichiers musicaux dans le m&ecirc;me dossier, pour &eacute;crire des billets</li>"
				."</ul>";
			echo "<li>Peut annuler son compte, mais laisse les images,les fichiers musicaux et le css sur le serveur.</li>"
			."</ul>";
			if( estCompteRelier($_SESSION['id_utilisateur']) ){
			...
			echo "<div class='conteneurGrand'>"
			."<h2 class='legende'>Renseignements G&eacute;n&eacute;raux :</h2>"
			."<h6 style='text-align:right;color:white;'>Ces renseignements sont ceux qui apparaissent dans la <a href='./index.php?page=associations'>liste</a> accessible sur l'interface publique du site.<br/>Ils sont n&eacute;cessaire pour que le compte soit valid&eacute;...<br />Votre compte a le statut :"
			."<span style='color:#".$couleur_status[$_SESSION['status_compte']].";'>".$status_compte[$_SESSION['status_compte']]."</span>"
			."</h6>";
			...
			if(isset($_GET['lecture'])){
				...
				if($association['description']=="" || $modif == "description"){
					...
					echo "DESCRIPTION : </td><td class='utilisateurs'>".$association['description']."</td><td class='utilisateurs'><a href='index.php?page=compte&modif=description&valeur=".$association['description']."#ancre_formulaire'>Modifier</a>";
				}
				echo "</td></tr><tr><td class='titreTableau'>";
...
/***************************************************************************************************************************************************************H4X0R*/	
	echo "Votre compte PIRATE ?!!!!!? .</h1>"
		."<ul class='preferences'>"
			."<li>Peut lire les billets des artistes, associations ainsi que les articles et les dossiers des journalistes</li>"
			."<li>Peut t&eacute;l&eacute;charger le CMS - OpenJournaliste - Sous-licence</li>"
				."<ul>"
				."<li>Peut demander &agrave; travailler avec la communaut&eacute;e</li>"
				."<li>Peut envoyer un mail &agrave; l'administrateur du site pour devenir journaliste &agrave; son tour</li>"
				."<li>Peut avoir acc&egrave;s aux <a href='index.php?page=archives'>archives</a> des articles...</li>"
				."<li>Peut acc&egrave;der &agrave; l' InterWeboGraphie du site.</li>"
				."</ul>"
			."<li>Peut chier partout, m&ecirc;me sur les murs si &ccedil;a lui chante! </li>"
			."</ul>";
