<?php
function AfficheArchiveArticlesDuJournaliste(){
	...
	while($article = mysql_fetch_assoc($req_articles)){
		echo "<td valign='top' align='center' class='article' >"
			...
			."<p> Lu ".AfficheNbLectureArticle($article['id_article'])." fois...</p> "
			...
			."<form method='get' action='controlleurs/traitementSuppressionArticle.php'>"
			."<input type='hidden' name='id' value='".$article['id_article']."'/><input type='submit' value='Supprimer'/>"
			."</form>"
			."<a href='index.php?page=correctionArticle&id=".$article['id_article']."'>Corriger La(Les) faute(s)</a>";
		echo "</td>";
