...
function AfficheFormEcritureDossier(){
...
echo wcode_css();
	if(isset($_GET['id'])){
		$data = mysql_fetch_assoc(recuperationDossierTemporaire($_GET['id']));
	}else{
		$data = array(	'titre' => '',
				'bbcode' => '',
				'corps' => '',
	...
	if(!isset($_GET['testMiseEnPage'])){
		echo "<form method='post' name='dossier' action='controlleurs/traitementDossiers.php'>"
		."<h2 class='utilisateurs'>&Eacute;criture d'un Dossier:</h3>"
		."<table border='0'><tr><td class='ecritureInverse'>"
		."Titre: </td><td class='ecriture'><input type='text' name='titre' size='100' value='".$data['titre']."'>"
		."</td></tr><tr><td class='ecritureInverse'>"
		."Description sommaire du dossier:</td><td class='ecriture'><input type='text' name='description' size='100' value='".$data['description']."'>"
		."</td></tr><tr><td class='ecriture' colspan='2'>";
		...
		if(isset($_POST['reecriture'])){
			echo "<input type='hidden' name='reecriture' value='oui'/>"
		...
		echo "<p class='corpsDossier'>";
		 if ($r){
				echo $wc->code_html;
				echo "</p><center>";
				echo "<form method='post' style='display:inline;' action='index.php?page=ecritureDossier&id=".$data['id_dossier']."#corpsPage'>"
				."<input type='hidden' name='reecriture' value='oui'/>"
				."<input type='submit' class='btn_dossiers' value='Modifier?' />"
				."</form>";
				echo "<form method='post' style='display:inline;' action='controlleurs/traitementDossiers.php'>"
					."<input type='hidden' name='validation_id' value='".$data['id_dossier']."'/>"
				...
				echo "<form method='post' action='index.php?page=ecritureDossier&id=".$data['id_dossier']."#corpsPage'>"
				."<input type='hidden' name='reecriture' value='oui'/>"
				."<input type='submit' class='btn_dossiers' value='Modifier!' />"
				."</form>";
		 }
