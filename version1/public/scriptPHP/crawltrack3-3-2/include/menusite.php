...
if( $varGetIncludePageWithRedirection == '' ){
	include dirname(__FILE__).'/menusite_original.php';
	return;
}else{
...
if ($_SESSION['rightsite'] == 0) {
	...
	if ($nbrresult >= 1) {
		...
		//display
		echo "<div style='clear:right;display:inline;float:right;' align=\"center\">\n";
	...
	if ($nbrresult >= 1) {
		...
		//display
		echo "<div  style='clear:right;display:inline;float:right;' >\n";
		echo "<form action=\"index.php".$varPostFormIncludePageWithRedirection."\" method=\"POST\" >\n";
		echo "<input type=\"hidden\" name ='navig' value=\"$navig\">\n";
		echo "<input type=\"hidden\" name ='search' value=\"$search\">\n";
		echo "<input type=\"hidden\" name ='period' value=\"$period\">\n";
