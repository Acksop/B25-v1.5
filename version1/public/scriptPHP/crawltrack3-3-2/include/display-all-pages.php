...
if ($nbrresult >= 1) {
  ...
  AND crawlt_visits.crawlt_site_id_site='" . sql_quote($site) . "'";
	...
	if ($graphpos == 0 && $period != 5) {
		...
		echo "<img src=\"".$pathFromRootToCrawltrackImages."graphs/page-graph.php?nbrpageview=$nbrtotpages&amp;nbrpagestotal=$nbrpagestotal&amp;crawltlang=$crawltlang\" alt=\"graph\"  style=\"border:0; width:500px; height:200px\">\n";
		echo "</a>\n";
		echo "</div><br>\n";
	}
	...
	if ($graphpos == 1 && $period != 5) {
		...
		echo "<img src=\"".$pathFromRootToCrawltrackImages."graphs/page-graph.php?nbrpageview=$nbrtotpages&amp;nbrpagestotal=$nbrpagestotal[$site]&amp;crawltlang=$crawltlang\" alt=\"graph\"  width=\"500px\" height=\"175px\" style=\"border:0\"/>\n";
		echo "</a>\n";
		echo "</div>\n";
	}
	...
	foreach ($sorttab as $key => $value) {
		if ($comptligne < ($rowdisplay + 2) || $displayall == 'yes') {
			...
			if ($comptligne % 2 == 0) {
				...
				echo ">&nbsp;&nbsp;<a href='index.php?".$varGetIncludePageWithRedirection."navig=4&amp;period=" . $period . "&amp;site=" . $site . "&amp;crawler=" . $crawlencode . "&amp;graphpos=" . $graphpos . "' rel='nofollow'>" . $crawldisplay . "</a></td>\n";
