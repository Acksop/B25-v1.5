...
class Fichier {
	var $nom;
	var $nomOriginal;
	var $type; //0:image ; 1:musique ; 2: document
	var $garderOriginal; //1:oui ; 0:non
	var $garderNom; //1:oui 0:non
	var $erreurs;
	var $destination;
	var $tampon;
	var $redimensionnement; //0:aucun ; 1:par la largeur ; 2:par la hauteur
	var $tailleMax;
	var $tailleX;
	var $tailleY;
	...
	function Fichier($tamponFichier,$RepDestination,$type,$redimentionnement,$tailleMax,$garderOriginal,$garderNom){
		...
		if(!isset($RepDestination)){
			$this->destination = "chemin_vers_les/upload_utilisateurs/";
		}else{
	...
	function testExtentionMIME(){
		...
		$mimeImage = array('image/jpeg','image/gif','image/png');
		$extImage = array('jpeg','jpg','JPEG','JPG','gif','GIF','PNG','png');
		$extMusique = array('mp3','MP3');
		$extDocument = array('txt','TXT','doc','docx','ppt','pdf');
		$mimeMusique = array('audio/mpeg');
		$mimeTexte = array('application/msword','application/pdf','application/mspowerpoint','text/plain');
	...
	function reecritureNom(){
		...
		$accents = array(
				"À","Á","Â","Ã","Ä","Å","à","á","â","ã","ä","å","Ò","Ó","Ô","Õ","Ö","Ø","ò","ó","ô","õ","ö","ø","È","É",
